...
get_role_all_host(){
    local role=$1
    if [ ! -f ${config} ]; then
        error_log "没有找到app的配置文件,请检查安装路径${file_root}下面是否存在${app_config}"
        exit 1
    fi
    cat ${config}|grep $role|grep -oP "(\d+\.){3}\d+"|sort -u
}
...
decompress_all(){
    ...
    echo "port ${port}" >> ${file_root}/step/config.conf
...
install_base(){
    echo "------------------开始安装base包-----------------"
    sudo cp -a ${file_root}/ip_template.json  ${install_path}/titan-base/service_ip.conf 
...
install_app(){
    ...
    #cd ${install_path}/titan-base && sudo bash titan-app.sh install v3
    check app安装步骤
}
