...
## glusterfs
QT_PACKAGE_DIR="/data/qt_base/glusterfs"
...
install_glusterfs(){
    info_log "Install glusterfs...."
    yum -y install glusterfs-server glusterfs
    yum -y update glusterfs-server glusterfs
...
launch_service(){
    ...
    check "start glusterd"
}
check_fstab(){
    local glusterfs_ip=$1
    [ -z "`grep glusterfs /etc/fstab`" ] && echo "${glusterfs_ip}:/java   /data/app/titan-dfs    glusterfs   defaults,_netdev 0 0" >> /etc/fstab || echo "fstab already installed glusterfs"
}
...
launch_service
ensure_dir_exists /data/storage
ensure_dir_exists /data/app/titan-dfs
