...
# 复制于 ansible-2.9.12/lib/ansible/plugins/callback/default.py 
# 因不好控制某些日志包含密码，需要仅打印在console，但是不记录在日志中，做了改造如下
# v2_runner_on_ok 函数中 161行左右，判断如果 vars 中包含 screen_only，则不输出到日志中，仅打印到console中
DOCUMENTATION = '''
    callback: titan_default
    type: stdout
    short_description: titan_default Ansible screen output
    version_added: historical
    description:
        - This is the titan_default output callback for ansible-playbook.
...
class CallbackModule(CallbackBase):
    ...
    '''
    This is the titan_default callback interface, which simply prints messages
    to stdout when new callback events are received.
    '''

    CALLBACK_VERSION = 2.0
    CALLBACK_TYPE = 'stdout'
    CALLBACK_NAME = 'titan_default'
    ...
    def v2_runner_on_failed(self, result, ignore_errors=False):
        ...
        self.check_stop_liveoutput(result)
    ...
    def v2_runner_on_ok(self, result):
        ...
        if result._task.loop and 'results' in result._result:
            ...
            if 'screen_only' in result._task.get_vars():
                #self._display.display(json.dumps(result._task.get_vars()), color=color, screen_only=True)
                self._display.display(msg, color=color, screen_only=True)
            else:
    ...
    def v2_playbook_on_task_start(self, task, is_conditional):
        ...
        if "live_output" in task.get_vars():
            live_log = task.get_vars()["live_output"]
            #self._display.display("live_output task start because v2_playbook_on_task_start", screen_only=False)
            self.start_tail(live_log)
    ...
    def check_stop_liveoutput(self, result):
        if "live_output" in result._task.get_vars():
            live_log = result._task.get_vars()["live_output"]
            #self._display.display("live_output task end because task end", screen_only=False)
            self.stop_tail(live_log)

    def start_tail(self, live_log):
        if live_log in self.logs_dict:
            self._display.display("live_log already in logs_dict", screen_only=False)
            return

        tail_thread = threading.Thread(target=self.tail, args=(live_log,))
        tail_thread.setDaemon(True)
        tail_thread.start()
        self.logs_dict[live_log] = True

    def stop_tail(self, live_log):
        if live_log in self.logs_dict:
            del self.logs_dict[live_log]
            time.sleep(0.25)   

    def tail(self, live_log):
        try_count = 0
        while True:
            time.sleep(0.3)
            if not live_log in self.logs_dict:
                return
            try:
                fp = open(live_log, 'r')
                break
            ...
            if try_count > 10:
                self._display.display("can not open live_log %s" % live_log, screen_only=False)
                return
        ...
        while True:
            ...
            if not live_log in self.logs_dict:
                ...
                #self._display.display("live_log output end", screen_only=False)
                return