...
func readSecret(fileName string) (string, error) {
	...
	plain, err := decryptPbe(pbeconfig, strings.TrimSpace(string(data)))
	return plain, err
}
...
// 给Merge用的，Merge之前先处理 properties 模板
func processPropertiesTemplate(cfg templateConfig, src string) (string, error) {
	option := "missingkey=default"
	if cfg.strict {
		option = "missingkey=error"
	}
	...
	err = tmpl.Execute(buffer, cfg.data)
