...
var (
	...
	cfg struct {
		...
		secretdir    string
		createSecret string
		encSecrets   stringsFlag // encSecretFile:plainFile
		delAfter     stringsFlag
		delDelay     time.Duration

		mergeToJson stringsFlag // properties:originJson:destJson
		mergeToIni  stringsFlag // properties:originIni:destIni
	}
	pbeconfig = ""
)
...
func init() { //nolint:gochecknoinits // By design.
	...
	flag.Var(&cfg.yamlEnvPaths, "yaml-env", "path to yaml file to load var into .Env")
	...
	flag.StringVar(&cfg.getPlain, "getPlain", "", "get `service` plain password")
	flag.StringVar(&cfg.encrypt, "encrypt", "", "encrypt `plainText` ")
	flag.StringVar(&cfg.pbePath, "pbePath", "", "`pbeconfig path`, default is /run/secrets/pbeconfig")
	flag.StringVar(&cfg.secretdir, "secretdir", "", "`secret dir`, default is /run/secrets")
	flag.StringVar(&cfg.createSecret, "createSecret", "", "`secret path`, will encrypt a secret and write to it")
	flag.Var(&cfg.encSecrets, "encSecret", "`encFile:plainFile`, will decrypt to plain file, can be passed multiple times")

	flag.Var(&cfg.delAfter, "delAfter", "`delAfter` iles will be delete after start, can be passed multiple times")
	flag.DurationVar(&cfg.delDelay, "delDelay", defDelDelay, "delat time for delAfter")

	flag.Var(&cfg.mergeToJson, "mergeToJson", "mergeToJson `properties:originJson:destJson` destJson can be empty, if destJson is empty, it will be originJson")
	flag.Var(&cfg.mergeToIni, "mergeToIni", "mergeToIni `properties:originIni:destIni`  destIni can be empty, if destIni is empty, it will be originIni")
...
func main() { //nolint:gocyclo,gocognit,funlen // TODO Refactor?
	...
	var iniURL, iniHTTP, templatePathBad, mergeToJsonBad, mergeToIniBad, encSecretsBad, delAfterBad, yamlEnvBad, waitBadScheme, waitHTTP bool
	...
	for _, path := range cfg.mergeToJson {
		const maxParts = 3
		parts := strings.Split(path, ":")
		mergeToJsonBad = mergeToJsonBad || path == "" || parts[0] == "" || parts[1] == "" || len(parts) > maxParts
	}
	for _, path := range cfg.mergeToIni {
		const maxParts = 3
		parts := strings.Split(path, ":")
		mergeToIniBad = mergeToIniBad || path == "" || parts[0] == "" || parts[1] == "" || len(parts) > maxParts
	}
	for _, path := range cfg.encSecrets {
		const maxParts = 2
		parts := strings.Split(path, ":")
		encSecretsBad = encSecretsBad || path == "" || parts[0] == "" || len(parts) > maxParts
	}
	for _, path := range cfg.delAfter {
		delAfterBad = delAfterBad || path == ""
	}
	for _, path := range cfg.yamlEnvPaths {
		yamlEnvBad = yamlEnvBad || path == ""
	}
	...
	case encSecretsBad:
		fatalFlagValue("require encFile:plainFile", "encSecret", cfg.encSecrets)
	case delAfterBad:
		fatalFlagValue("require delAfterFile", "delAfter", cfg.delAfter)
	case yamlEnvBad:
		fatalFlagValue("require yamlEnvPath", "yaml-env", cfg.yamlEnvPaths)
	...
	if cfg.secretdir == "" {
		cfg.secretdir = defaultSecretDir
	}

	if cfg.pbePath == "" {
		cfg.pbePath = path.Join(cfg.secretdir, "pbeconfig")
	}
	if pbeSecret, err := ioutil.ReadFile(cfg.pbePath); err == nil {
		pbeconfig = string(pbeSecret)
	}

	if cfg.encrypt != "" {
		doEncrypt(string(pbeconfig), cfg.encrypt)
		os.Exit(0)
	}

	if cfg.getPlain != "" {
		doGetPlain(string(pbeconfig), cfg.secretdir, cfg.getPlain)
		os.Exit(0)
	}

	if cfg.createSecret != "" {
		err := createSecretFile(cfg.createSecret)
		if err != nil {
			fmt.Println(err)
			os.Exit(1)
		}
	...
	// 处理 secret 将其解密成明文写入 plainFile
	err = processEncSecrets(cfg.encSecrets)
	if err != nil {
		fatalf("Failed to process encSecret: %s.", err)
	}
	...
	// 从 titan-env.yaml 读取配置作为变量
	for _, path := range cfg.yamlEnvPaths {
		loadEnvFromYaml(cfg.template.data.Env, path)
	}
	...
	// 处理 java.json/build.json
	processMergeToJson(cfg.template, cfg.mergeToJson)

	// 处理 application.ini
	processMergeToIni(cfg.template, cfg.mergeToIni)
	...
	case len(cfg.tailStdout)+len(cfg.tailStderr) > 0:
		...
		select {}
	default:
		if len(cfg.delAfter) > 0 {
			delAfterDelay()
			time.Sleep(cfg.delDelay + 3)
		}
	}
}
...
func delAfterDelay() {
	if len(cfg.delAfter) > 0 {
		println("delAfter:" + strings.Join(cfg.delAfter, " "))
		go func() {
			fmt.Println("begin wait to delete file")
			time.Sleep(cfg.delDelay)
			for _, path := range cfg.delAfter {
				os.Remove(path)
			}
		}()
