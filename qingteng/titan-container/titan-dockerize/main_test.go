...
func TestLoadYaml(tt *testing.T) {
	env := getEnv()
	loadEnvFromYaml(env, "/data/code/titan-container/titan-deploy-on-k8s/titan-env.yml")
	for k, v := range env {
		fmt.Println(k + ":" + v)
	}
}
...
func TestFlag(tt *testing.T) {
	...
	cases := []struct {
		...
		{[]string{"-wait", ""}, `file/tcp/tcp4/tcp6/unix/http/https`},
		{[]string{"-wait", "/dev/null"}, `file/tcp/tcp4/tcp6/unix/http/https`},
